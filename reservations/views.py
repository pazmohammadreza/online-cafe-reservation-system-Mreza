...
def pending_list(request):
    ...
    return render(request, 'status_list.html', {'reservations': res, 'title': 'pending'})

@login_required
def confirmed_list(request):
    res = Reservation.objects.filter(user=request.user, status=Status.CONFIRMED)
    return render(request, 'status_list.html', {'reservations': res, 'title': 'confirmed'})

@login_required
def cancelled_list(request):
    res = Reservation.objects.filter(user=request.user, status=Status.CANCELLED)
    return render(request, 'status_list.html', {'reservations': res, 'title': 'cancelled'})

@login_required
def completed_list(request):

    res = Reservation.objects.filter(user=request.user, status=Status.COMPELETED)
    return render(request, 'status_list.html', {'reservations': res, 'title': 'completed'})
